...
*/
    ...
    // https://medium.com/building-the-system/gunicorn-3-means-of-concurrency-efbb547674b7
    // https://stackoverflow.com/a/59183741/2254192
    const websocketApiContainer = testnetWebsocket.addContainer("websockets", {
      memoryLimitMiB: 512,
      cpu: 512, // 1vCPU
      image: ecs.ContainerImage.fromRegistry(
        `657117221658.dkr.ecr.eu-west-1.amazonaws.com/${ecrRepository.repositoryName}:latest`
      ),
      containerName: "websockets",
      environment: {
        TIMEOUT: "600",
        ENV: "testnet",
        //DB_URL: `mongodb://spr:spr_password@sprdbmongo45760d76-pqebhg876v2s.cluster-cch2nm8vz0as.eu-west-1.docdb.amazonaws.com:27017`,
...
*/
    const testnetListenerSsl = lb.addListener("MainnetListenerSsl", {
      port: 444,
      certificates: [this.apiSprCertificate()],
      protocol: elb.ApplicationProtocol.HTTPS
    });
...
*/
    testnetListenerSsl.addTargetGroups("TestnetTargetGroup", {
...
*/
      ...
      actions: [
        new cpa.CodeBuildAction({
          ...
          environmentVariables: {
            BASE_API_PATH: { value: "https://api.spaceriders.io:444" },
    ...
    const imgDef3 = new cp.Artifact("websockets");
    
    apipipeline.addStage({
      stageName: "Build",
      actions: [
        new cpa.CodeBuildAction({
          outputs: [imgDef1, imgDef2, imgDef3],
          actionName: "Build",
    ...
    apipipeline.addStage({
      ...
      actions: [
        ...
        new cpa.EcsDeployAction({
          actionName: "Websockets",
          service: websocketServiceTestnet,
          //input: imgDef2,
          imageFile: imgDef3.atPath('imagedefinitions3.json'),
          deploymentTimeout: cdk.Duration.minutes(15),
        }), 
      ],
    });
